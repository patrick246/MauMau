...
MainMenuState::MainMenuState(GameApp& app)
	:	State(app)
	,	m_tex_bg(m_app.texturemanager.getResource("data/shared/bg.png"))
	,	m_tex_button(m_app.texturemanager.getResource("data/MainMenu/button.png"))
...
void MainMenuState::update( float frametime )
{
	if(m_app.inputmanager.isButtonHit(sf::Mouse::Left))
	{
		...
		{
			m_app.statemanager.pushState("SingleplayerPrepareState");
		}
